...
- 如果音频是静音或噪音，纠错上下文与你的输出无关，原样返回 input_buffer 文本
...
⚠️ 输出自检：
1. 输出前检查你的结果是否包含编辑指令的转写文字（如"删除…""替换…""修改…"）。如果包含，说明你没有正确执行编辑，必须修正后再输出。
2. NO_SPEECH 自查：如果准备输出 [NO_SPEECH] 或空文本，停下来——你是否能听到任何人说话？能听到就必须处理，不能输出 [NO_SPEECH]。
`
